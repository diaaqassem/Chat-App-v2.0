    ...
    OnApplicationShutdown
{
  ...
  async removeFromAllRooms(clientId: string) {
    try {
  ...
  async createRoom(client: Socket, data: CreateRoomDto) {
    ...
    try {
      ...
      const joinRoom = this.customI18nService.translate('gateway.CREATE_ROOM');
      this.server.to(client.id).emit('joined_room', `${joinRoom}: ${room}`);
